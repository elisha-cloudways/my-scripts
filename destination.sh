...
get_db(){
        read -p 'Destination DB name: ' db
}

get_pw(){
        read -p 'Destination DB password: ' pw
}
get_db;
get_pw;
...
until [[ $dbaccess = "success" ]]; do
        ...
        mysql --user="${db}" --password="${pw}" -e exit 2>/dev/null
        dbstatus=`echo $?`
        if [ $dbstatus -ne 0 ]; then
                #echo -e "Destination MySQL Username [$db]: \c "
                #read dbuser
...
echo "Updating WP-config details..."
