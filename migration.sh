...
#Save prefix in conf-data
echo "prefix|$prefix" > conf-data.txt

#Save db name in conf-data
echo "dest_db|$dest_db" >> conf-data.txt

# To fetch db name on destination
grep db_name conf-data.txt | cut -d "|" -f 2

# To fetch prefix on destination
grep prefix conf-data.txt | cut -d "|" -f 2

# Execute commands on remote server
ssh -T master_bycshgmnke@172.105.162.236 >/dev/null 2>&1 << "ENDSSH"
touch success-dest.txt
touch silent-mufu.txt
cd applications/
ls
ENDSSH

# back to source server
touch success.txt
echo "This shit is lit"
